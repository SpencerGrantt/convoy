...
export default function PhotoCapture({ runId, companyId, photoType, existingPath, onCaptured }) {
  ...
  const [mode, setMode] = useState('idle') // idle | camera | preview | done
  ...
  // If photo already exists, show done state
  useEffect(() => {
    if (existingPath) setMode('done')
  }, [existingPath])
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      {/* Done — show checkmark placeholder (signed URLs load slow, skip for now) */}
      {mode === 'done' && (
      ...
      <div className="px-4 py-3">
        ...
        {mode === 'camera' && (
          <button
            ...
            className="w-full bg-gray-100 text-gray-600 font-semibold py-2.5 rounded-xl text-sm"
          >
            Cancel
          </button>
        )}
        ...
        {mode === 'done' && (
          <button
            onClick={() => { setMode('idle'); setPreviewUrl(null) }}
            className="w-full bg-gray-100 text-gray-500 font-medium py-2 rounded-xl text-xs"
          >
            Retake
          </button>
        )}
